    ...
    def __init__(self, dataset_root, output_dir, epochs=100, batch_size=16, img_size=640, max_images_per_dataset=None):
        ...
        self.dataset_paths = {
            ...
            'Czech': {
                ...
                'test_images': '/kaggle/input/rdd2022-more/RDD2022_Czech/Czech/test/images'
            },
            'China_MotorBike': {
            ...
            'Japan': {
                ...
                'test_images': '/kaggle/input/rdd2022-more/RDD2022_Japan/Japan/test/images',
            },
        }
        ...
        if self.max_images_per_dataset:
            ...
            print(f"📁 Loading all available images from datasets")
...
def main():
    """Main entry point"""
    # ========================================================================
    # 🔧 CONFIGURATION - Chỉnh sửa các tham số ở đây
    # ========================================================================
    
    # Dataset paths (Kaggle)
    DATASET_ROOT = '/kaggle/input'  # Thư mục chứa datasets
    OUTPUT_DIR = '/kaggle/working/outputs'  # Thư mục lưu kết quả
    
    # Training parameters
    EPOCHS = 50  # Số epochs (khuyến nghị: 50-100 cho Kaggle)
    BATCH_SIZE = 16  # Batch size (giảm xuống 8 nếu hết RAM)
    IMG_SIZE = 640  # Kích thước ảnh (640x640 là chuẩn)
    
    # Model selection
    # Chọn một trong: 'yolov8n.pt', 'yolov8s.pt', 'yolov8m.pt', 'yolov8l.pt', 'yolov8x.pt'
    # yolov8n: nhanh nhất, nhẹ nhất (cho test)
    # yolov8s: nhỏ, nhanh
    # yolov8m: trung bình - KHUYẾN NGHỊ cho Kaggle
    # yolov8l: lớn, chính xác cao
    # yolov8x: lớn nhất, chính xác nhất (có thể chậm trên Kaggle)
    MODEL_NAME = 'yolov8m.pt'
    
    # ⭐ Giới hạn dataset (quan trọng cho Kaggle)
    # None = không giới hạn (sử dụng toàn bộ dataset)
    # 500 = chỉ lấy 500 ảnh từ mỗi dataset
    # 100 = chỉ lấy 100 ảnh (dùng để test nhanh)
    MAX_IMAGES_PER_DATASET = 500  # Đổi thành None để không giới hạn
    
    # ========================================================================
    # 🚀 TRAINING - Không cần chỉnh sửa phần này
    # ========================================================================
    ...
    print(f"⚠️  Max images/dataset: {MAX_IMAGES_PER_DATASET if MAX_IMAGES_PER_DATASET else 'No limit'}")
    print("="*70 + "\n")
    
    # Kiểm tra dataset tồn tại
    if not os.path.exists(DATASET_ROOT):
        print(f"❌ ERROR: Dataset root not found: {DATASET_ROOT}")
        print(f"Please update DATASET_ROOT in the configuration section")
        return
